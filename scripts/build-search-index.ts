...
import { createSearchIndex } from "../src/search/search-index.ts";
...
export const searchIndexArtifactPath = path.join(
  projectRoot,
  "src/search/search-index.gen.json",
);
...
if (import.meta.filename === process.argv[1]) {
  ...
  const demoCount = components.reduce(
    (total, component) => total + component.demos.length,
    0,
  );
