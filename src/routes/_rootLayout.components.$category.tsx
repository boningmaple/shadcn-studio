...
export const Route = createFileRoute("/_rootLayout/components/$category")({
  staticData: { ariaLabel: "Component Collection" },
  loader: async ({ params }) => {
    const group = await getRegistryItemGroup({
      data: { category: params.category, type: "registry:component" },
    });

    if (group === null) throw notFound();
    return group;
  },
  component: ComponentCollectionPage,
});

function ComponentCollectionPage() {
    ...
    <div className="mx-auto w-full max-w-7xl space-y-12 py-6">
      ...
      <div className="space-y-16">
        {group.items.map((item) => (
          <RegistryComponentShowcase item={item} key={item.name} />
        ))}
      </div>
    </div>
  );
...
function RegistryComponentShowcase({ item }: { item: RegistryItemShowcase }) {
