    ...
    <div className="mx-auto w-full max-w-7xl space-y-12 py-6">
      ...
      <div className="space-y-16">
        {group.items.map((item) => (
          <RegistryBlockShowcase item={item} key={item.name} />
        ))}
      </div>
    </div>
  );
...
function RegistryBlockShowcase({ item }: { item: RegistryItemShowcase }) {
