...
type NavigationBarSlotProps = Parameters<
  ReturnType<typeof navigationBarStyles>["bar"]
>[0];
...
export type NavigationBarProps = React.ComponentPropsWithRef<"nav">;

export function NavigationBar({ className, ...props }: NavigationBarProps) {
  const { bar } = navigationBarVariants();
...
export type NavigationBarBadge = React.ReactElement | number | string | true;
...
export function NavigationBarItem({
  ...
  const { icon, indicator, item, label } = navigationBarVariants();
  const renderedIcon =
  ...
  return (
    ...
    >
      <span className={indicator()} data-slot="navigation-bar-indicator">
        <BadgeAnchor className={icon()} data-slot="navigation-bar-icon">
          {renderedIcon}
          {renderBadge(badge)}
        </BadgeAnchor>
      </span>
...
function renderBadge(badge: NavigationBarItemProps["badge"]) {
