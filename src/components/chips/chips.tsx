...
type ChipSurface = NonNullable<ChipVariantProps["surface"]>;
...
export function ChipGroup({
  ...
  const { group } = chipVariants({ surface, variant });
...
export type ChipListProps<T extends object> = RACTagListProps<T> &
...
export function ChipList<T extends object>({
  ...
  const { list } = chipVariants({ surface, variant });
...
export type ChipProps = React.ComponentPropsWithRef<typeof RACTag> & {
...
export function Chip({
  ...
  const { icon, item, label, selectionIndicator, surface } = chipVariants({
