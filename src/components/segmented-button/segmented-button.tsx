...
export const segmentedButtonStyles = tv({
  slots: {
    group: [
...
export type SegmentedButtonVariantProps = VariantProps<
  typeof segmentedButtonStyles
>;
type SegmentedButtonSlotProps = Parameters<
  ReturnType<typeof segmentedButtonStyles>["group"]
>[0];

export function segmentedButtonVariants({
  density = "default",
  equalWidth = false,
}: SegmentedButtonVariantProps = {}) {
  const resolve = () => segmentedButtonStyles({ density, equalWidth });
...
type SegmentedButtonStyleContextValue = {
  density: NonNullable<SegmentedButtonVariantProps["density"]>;
  equalWidth: NonNullable<SegmentedButtonVariantProps["equalWidth"]>;
};
...
export type SegmentedButtonGroupProps = React.ComponentPropsWithRef<
...
export function SegmentedButtonGroup({
  ...
  const { group } = segmentedButtonVariants({ density, equalWidth });
...
export function SegmentedButton({
  ...
  const {
    ...
    content,
    icon: iconClassName,
  } = segmentedButtonVariants({ density, equalWidth });
