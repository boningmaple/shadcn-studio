...
type DialogSlotProps = Parameters<ReturnType<typeof dialogStyles>["dialog"]>[0];
...
export function Dialog({
  ...
  const { dialog, modal, overlay } = dialogVariants();
...
export type DialogCloseProps = React.ComponentPropsWithRef<typeof Button>;
...
export function DialogClose({
  ...
  return (
    <Button slot={slot} variant={variant} {...props}>
      {children}
    </Button>
  );
}
...
export type FullScreenDialogBarProps = React.ComponentPropsWithRef<"div"> & {
  showCloseButton?: boolean;
};

export function FullScreenDialogBar({
  children,
  className,
  showCloseButton = true,
  ...props
}: FullScreenDialogBarProps) {
  const { fullScreenBar } = dialogVariants();
  ...
  return (
    ...
    >
      {showCloseButton ? (
        <IconButton aria-label="Close dialog" slot="close" variant="standard">
          <XIcon />
        </IconButton>
      ) : null}
      {children}
...
export type FullScreenDialogTitleProps = DialogTitleProps;

export function FullScreenDialogTitle({
  className,
  ...props
}: FullScreenDialogTitleProps) {
  const { fullScreenTitle } = dialogVariants();
...
export type FullScreenDialogContentProps = DialogContentProps;

export function FullScreenDialogContent({
  className,
  ...props
}: FullScreenDialogContentProps) {
  const { fullScreenContent } = dialogVariants();
