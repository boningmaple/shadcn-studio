...
type TabsOrientation = NonNullable<TabsVariantProps["orientation"]>;
...
export function Tabs({
  ...
  const { root } = tabsVariants({ orientation, variant });
...
export function TabList<T extends object>({
  ...
  const { list } = tabsVariants({ orientation, variant });
...
export type TabProps = React.ComponentPropsWithRef<typeof RACTab>;

export function Tab({ children, className, ...props }: TabProps) {
  const { orientation, variant } = React.useContext(TabsStyleContext);
  const { content, indicator, tab } = tabsVariants({
    orientation,
    variant,
  });
...
export type TabPanelsProps<T extends object> = RACTabPanelsProps<T> &
  React.RefAttributes<HTMLDivElement>;
...
export function TabPanels<T extends object>({
  ...
  const { orientation, variant } = React.useContext(TabsStyleContext);
  const { panels } = tabsVariants({ orientation, variant });
...
export type TabPanelProps = React.ComponentPropsWithRef<typeof RACTabPanel>;

export function TabPanel({ className, ...props }: TabPanelProps) {
  const { orientation, variant } = React.useContext(TabsStyleContext);
  const { panel } = tabsVariants({ orientation, variant });
