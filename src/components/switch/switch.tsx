...
type SwitchSlotProps = Parameters<ReturnType<typeof switchStyles>["root"]>[0];
...
export type SwitchProps = React.ComponentPropsWithRef<typeof RACSwitch>;

export function Switch({ children, className, ...props }: SwitchProps) {
  const { control, handle, icon, label, root, track } = switchVariants();
