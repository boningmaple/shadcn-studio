...
export const extendedFABButtonVariants = tv({
  base: [
...
export type ExtendedFABButtonProps = Omit<
  ButtonProps,
  "shape" | "size" | "variant"
> &
  VariantProps<typeof extendedFABButtonVariants>;

export function ExtendedFABButton({
  className,
  color = "surface",
  lowered = false,
  ...props
}: ExtendedFABButtonProps) {
  return (
    <Button
      className={composeRenderProps(className, (className) =>
        extendedFABButtonVariants({ className, color, lowered }),
      )}
      data-color={color}
