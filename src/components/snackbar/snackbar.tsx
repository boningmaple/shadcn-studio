...
type SnackbarSlotProps = Parameters<
  ReturnType<typeof snackbarStyles>["root"]
>[0];
...
export type SnackbarRegionProps<T extends SnackbarContent = SnackbarContent> =
...
export function SnackbarRegion<T extends SnackbarContent = SnackbarContent>({
  children,
  className,
  ...props
}: SnackbarRegionProps<T>) {
  const { region } = snackbarVariants();
  ...
  return (
    ...
    >
      {children ??
        (({ toast }: { toast: QueuedToast<T> }) => <Snackbar toast={toast} />)}
    </RACToastRegion>
  );
}

export type SnackbarProps<T extends SnackbarContent = SnackbarContent> = Omit<
  RACToastProps<T>,
  "children" | "className"
> & {
...
export function Snackbar<T extends SnackbarContent = SnackbarContent>({
  className,
  toast,
  ...props
}: SnackbarProps<T>) {
  const { action, close, content, message, root } = snackbarVariants();
  ...
  return (
    ...
    >
      ...
      {snackbar.actionLabel === undefined ? null : (
        <Button className={action()} onPress={handleAction} variant="text">
