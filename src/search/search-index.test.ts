...
describe("findHits", () => {
  ...
  it("orders the three indexed fields by their boosts", async () => {
    ...
    expect(labels.indexOf("Button")).toBeLessThan(
      labels.indexOf("Elevated button states"),
    );
    expect(labels.indexOf("Elevated button states")).toBeLessThan(
      labels.indexOf("Chips"),
    );
  });
  ...
  it("answers an empty query with every Component and no Demo", async () => {
    ...
    expect(hits.every((hit) => hit.kind === "component")).toBe(true);
  });
