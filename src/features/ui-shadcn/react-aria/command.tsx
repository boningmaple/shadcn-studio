...
import { InputGroup, InputGroupAddon } from "@/features/ui-shadcn/react-aria/input-group";
...
function CommandList<T extends object>({ className, ...props }: MenuProps<T>) {
  return (
    ...
    >
      <Menu {...props} data-slot="command-list" className="outline-none" />
    </div>
  );
}
...
function CommandGroup<T extends object>({
  ...
  return (
    <MenuSection
      ...
      className={cn(
        "overflow-hidden p-1 text-foreground **:[[cmdk-group-heading]]:px-2 **:[[cmdk-group-heading]]:py-1.5 **:[[cmdk-group-heading]]:text-xs **:[[cmdk-group-heading]]:font-medium **:[[cmdk-group-heading]]:text-foreground/70",
        className,
      )}
      {...props}
    >
