...
} from "@/features/ui-shadcn/react-aria/card";
...
export const mdCardStyles = tv({
  slots: {
    ...
    content: "relative z-10 px-4 py-4 text-sm leading-5 text-[#49454f] dark:text-[#cac4d0]",
  ...
  variants: {
    variant: {
      ...
      outlined: {
        root: ["border-[#cac4d0] bg-[#fffbfe]", "dark:border-[#49454f] dark:bg-[#1d1b20]"],
      },
    },
  },
  defaultVariants: {
...
export function mdCardVariants({ variant = "elevated" }: MDCardVariantProps = {}) {
...
export type MDCardHeaderProps = React.ComponentPropsWithRef<typeof BaseCardHeader>;

export function MDCardHeader({ className, ...props }: MDCardHeaderProps) {
  const { header } = mdCardVariants();

  return <BaseCardHeader className={header({ className })} data-slot="card-header" {...props} />;
}

export type MDCardContentProps = React.ComponentPropsWithRef<typeof BaseCardContent>;

export function MDCardContent({ className, ...props }: MDCardContentProps) {
  const { content } = mdCardVariants();

  return <BaseCardContent className={content({ className })} data-slot="card-content" {...props} />;
}
...
export function MDCardActions({ className, ...props }: MDCardActionsProps) {
  ...
  return <div className={cn(actions(), className)} data-slot="card-actions" {...props} />;
}
