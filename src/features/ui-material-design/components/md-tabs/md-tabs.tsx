...
const tabsStyles = tv({
  slots: {
    root: ["w-full bg-[#fffbfe] text-[#1d1b20]", "dark:bg-[#1d1b20] dark:text-[#e6e0e9]"],
...
export type MDTabsVariantProps = VariantProps<typeof tabsStyles>;
type TabsVariant = NonNullable<MDTabsVariantProps["variant"]>;
type TabsOrientation = NonNullable<MDTabsVariantProps["orientation"]>;
type TabsSlotProps = Parameters<ReturnType<typeof tabsStyles>["root"]>[0];

export function mdTabsVariants({
  orientation = "horizontal",
  variant = "primary",
}: MDTabsVariantProps = {}) {
...
export type MDTabsProps = Omit<React.ComponentPropsWithRef<typeof RACTabs>, "orientation"> &
  MDTabsVariantProps;
...
export function MDTabs({
  ...
  const { root } = mdTabsVariants({ orientation, variant });
...
export type MDTabListProps<T extends object> = RACTabListProps<T> &
  React.RefAttributes<HTMLDivElement>;

export function MDTabList<T extends object>({ className, ...props }: MDTabListProps<T>) {
  const { orientation, variant } = React.useContext(TabsStyleContext);
  const { list } = mdTabsVariants({ orientation, variant });
...
export type MDTabProps = React.ComponentPropsWithRef<typeof RACTab>;

export function MDTab({ children, className, ...props }: MDTabProps) {
  const { orientation, variant } = React.useContext(TabsStyleContext);
  const { content, indicator, tab } = mdTabsVariants({
    orientation,
    variant,
  });
  ...
  return (
    <RACTab
      data-slot="tab"
      className={composeRenderProps(className, (className) => tab({ className }))}
      {...props}
    >
      {composeRenderProps(children, (children) => (
        <span className={content()} data-slot="tab-content">
          {children}
          <RACSelectionIndicator className={indicator()} data-slot="tab-indicator" />
        </span>
      ))}
    </RACTab>
  );
...
export type MDTabPanelsProps<T extends object> = RACTabPanelsProps<T> &
  React.RefAttributes<HTMLDivElement>;

export function MDTabPanels<T extends object>({ className, ...props }: MDTabPanelsProps<T>) {
  const { orientation, variant } = React.useContext(TabsStyleContext);
  const { panels } = mdTabsVariants({ orientation, variant });

  return <RACTabPanels className={panels({ className })} data-slot="tab-panels" {...props} />;
}

export type MDTabPanelProps = React.ComponentPropsWithRef<typeof RACTabPanel>;

export function MDTabPanel({ className, ...props }: MDTabPanelProps) {
  const { orientation, variant } = React.useContext(TabsStyleContext);
  const { panel } = mdTabsVariants({ orientation, variant });

  return (
    <RACTabPanel
      data-slot="tab-panel"
      className={composeRenderProps(className, (className) => panel({ className }))}
      {...props}
    />
  );
}
