...
export const mdMenuStyles = tv({
  slots: {
    ...
    description: "truncate text-xs leading-4 text-[#49454f] dark:text-[#cac4d0]",
    keyboard: "relative z-10 text-xs leading-4 text-[#49454f] dark:text-[#cac4d0]",
...
export type MDMenuTriggerProps = React.ComponentPropsWithRef<typeof RACMenuTrigger>;
...
export type MDMenuProps = Omit<React.ComponentPropsWithRef<typeof RACMenu>, "className"> &
  MenuPopoverProps &
...
export function MDMenu({
  ...
  return (
    ...
    >
      <RACMenu
        className={composeRenderProps(className, (className) => menu({ className }))}
        data-slot="menu"
        {...props}
      />
    </RACPopover>
...
export type MDMenuItemProps = Omit<React.ComponentPropsWithRef<typeof RACMenuItem>, "children"> & {
...
export function MDMenuSection({ children, className, title, ...props }: MDMenuSectionProps) {
  const { header, section } = mdMenuVariants();

  return (
    <RACMenuSection className={section({ className })} data-slot="menu-section" {...props}>
...
export type MDMenuSeparatorProps = React.ComponentPropsWithRef<typeof RACSeparator>;

export function MDMenuSeparator({ className, ...props }: MDMenuSeparatorProps) {
  const { separator } = mdMenuVariants();

  return (
    <RACSeparator className={separator({ className })} data-slot="menu-separator" {...props} />
  );
}

export type MDSubmenuProps = React.ComponentPropsWithRef<typeof RACSubmenuTrigger>;
