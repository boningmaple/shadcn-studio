...
export const mdDialogStyles = tv({
  slots: {
    ...
    content: "overflow-y-auto px-6 py-4 text-sm leading-5 text-[#49454f] dark:text-[#cac4d0]",
    ...
    fullScreenTitle: "font-heading flex-1 text-xl leading-7 font-normal tracking-normal",
    fullScreenContent:
      "overflow-y-auto px-6 py-6 text-sm leading-6 text-[#49454f] dark:text-[#cac4d0]",
  },
});

type DialogSlotProps = Parameters<ReturnType<typeof mdDialogStyles>["dialog"]>[0];
...
export function mdDialogVariants() {
  ...
  return {
    ...
    fullScreenContent: (props?: DialogSlotProps) => styles.fullScreenContent(props),
    fullScreenDialog: (props?: DialogSlotProps) => styles.fullScreenDialog(props),
    fullScreenModal: (props?: DialogSlotProps) => styles.fullScreenModal(props),
    fullScreenOverlay: (props?: DialogSlotProps) => styles.fullScreenOverlay(props),
...
export type MDDialogTitleProps = Omit<React.ComponentPropsWithRef<typeof RACHeading>, "slot">;

export function MDDialogTitle({ className, ...props }: MDDialogTitleProps) {
  const { title } = mdDialogVariants();

  return (
    <RACHeading className={title({ className })} data-slot="dialog-title" slot="title" {...props} />
  );
}
...
export function MDDialogContent({ className, ...props }: MDDialogContentProps) {
  ...
  return <div className={content({ className })} data-slot="dialog-content" {...props} />;
}
...
export function MDDialogActions({ className, ...props }: MDDialogActionsProps) {
  ...
  return <div className={actions({ className })} data-slot="dialog-actions" {...props} />;
}
...
export function MDFullScreenDialog({
  ...
  const { fullScreenDialog, fullScreenModal, fullScreenOverlay } = mdDialogVariants();
  ...
  return (
    <RACModalOverlay
      ...
      {...props}
    >
      <RACModal className={fullScreenModal({ className })} data-slot="full-screen-dialog-modal">
        <RACDialog className={fullScreenDialog()} data-slot="full-screen-dialog">
          {children}
        </RACDialog>
      </RACModal>
    </RACModalOverlay>
...
export function MDFullScreenDialogBar({
  ...
  return (
    <div className={fullScreenBar({ className })} data-slot="full-screen-dialog-bar" {...props}>
      {showCloseButton ? (
...
export function MDFullScreenDialogTitle({ className, ...props }: MDFullScreenDialogTitleProps) {
...
export function MDFullScreenDialogContent({ className, ...props }: MDFullScreenDialogContentProps) {
...
export function MDDialogActionButton({
  ...
  return (
    <MDButton
      className={composeRenderProps(className, (className) => cn("min-w-16", className))}
      variant="text"
      {...props}
    />
  );
