...
export const mdCheckboxStyles = tv({
  slots: {
    root: [
...
export type MDCheckboxVariantProps = VariantProps<typeof mdCheckboxStyles>;
type CheckboxSlotProps = Parameters<ReturnType<typeof mdCheckboxStyles>["root"]>[0];

export function mdCheckboxVariants() {
  const styles = mdCheckboxStyles();
...
export type MDCheckboxProps = React.ComponentPropsWithRef<typeof RACCheckbox>;

export function MDCheckbox({ children, className, ...props }: MDCheckboxProps) {
  const { control, icon, indicator, label, root } = mdCheckboxVariants();
...
type CheckboxGroupSlotProps = Parameters<ReturnType<typeof checkboxGroupStyles>["group"]>[0];

export function mdCheckboxGroupVariants() {
...
export type MDCheckboxGroupProps = React.ComponentPropsWithRef<typeof RACCheckboxGroup>;

export function MDCheckboxGroup({ className, ...props }: MDCheckboxGroupProps) {
  const { group } = mdCheckboxGroupVariants();

  return (
    <RACCheckboxGroup
      className={composeRenderProps(className, (className) => group({ className }))}
...
export type MDCheckboxGroupLabelProps = React.ComponentPropsWithRef<typeof RACLabel>;

export function MDCheckboxGroupLabel({ className, ...props }: MDCheckboxGroupLabelProps) {
  const { label } = mdCheckboxGroupVariants();

  return <RACLabel className={label({ className })} data-slot="checkbox-group-label" {...props} />;
}

export type MDCheckboxGroupDescriptionProps = React.ComponentPropsWithRef<typeof RACText>;

export function MDCheckboxGroupDescription({
  className,
  slot = "description",
  ...props
}: MDCheckboxGroupDescriptionProps) {
  const { description } = mdCheckboxGroupVariants();
...
export type MDCheckboxGroupErrorProps = React.ComponentPropsWithRef<typeof RACFieldError>;

export function MDCheckboxGroupError({ className, ...props }: MDCheckboxGroupErrorProps) {
  const { error } = mdCheckboxGroupVariants();

  return (
    <RACFieldError
      className={composeRenderProps(className, (className) => error({ className }))}
