...
export const mdSliderStyles = tv({
  slots: {
    root: [
    ...
    rail: ["pointer-events-none absolute rounded-full bg-[#e6e0e9]", "dark:bg-[#49454f]"],
    fill: [
...
export type MDSliderVariantProps = VariantProps<typeof mdSliderStyles>;
type SliderSlotProps = Parameters<ReturnType<typeof mdSliderStyles>["root"]>[0];

export function mdSliderVariants() {
  const styles = mdSliderStyles();
...
export type MDSliderProps = React.ComponentPropsWithRef<typeof RACSlider> & {
...
export function MDSlider({
  ...
  const { fill, header, label, output, rail, root, stop, thumb, tick, track, valueIndicator } =
    mdSliderVariants();
  ...
  return (
    ...
    >
      {composeRenderProps(children, (children, { orientation, state }) => {
        ...
        const activeOffset = state.values.length > 1 ? state.getThumbPercent(0) : 0;
        ...
        return (
          <>
            {hasVisibleLabel ? (
              <div className={header({ orientation })} data-slot="slider-header">
                <RACLabel className={label()} data-slot="slider-label">
                  {children}
                </RACLabel>
                <RACSliderOutput className={output()} data-slot="slider-output">
                  {state.values.map((_, index) => state.getThumbValueLabel(index)).join(" - ")}
                </RACSliderOutput>
              </div>
            ) : null}
            <RACSliderTrack className={track({ orientation })} data-slot="slider-track">
              <span className={rail({ orientation })} data-slot="slider-rail" />
              <RACSliderFill
                className={(renderProps) => fill({ orientation: renderProps.orientation })}
                data-slot="slider-fill"
                offset={activeOffset}
              />
              {showStopIndicators ? (
                <SliderStopIndicators className={stop({ orientation })} orientation={orientation} />
              ) : null}
...
export type MDRangeSliderProps = MDSliderProps;

export function MDRangeSlider(props: MDRangeSliderProps) {
  return <MDSlider {...props} />;
}
...
function SliderTick({
  ...
  const style =
    orientation === "vertical" ? { bottom: `${percent * 100}%` } : { left: `${percent * 100}%` };
...
function getSliderTickValues(minValue: number, maxValue: number, step: number) {
  ...
  return Array.from({ length: tickCount - 1 }, (_, index) => minValue + step * (index + 1));
}
