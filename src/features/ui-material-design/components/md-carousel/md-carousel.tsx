...
import {
  ...
  useCarousel,
} from "@/features/ui-shadcn/react-aria/carousel";
...
export type MDCarouselContentProps = React.ComponentPropsWithRef<typeof BaseCarouselContent> &
  Pick<CarouselVariantProps, "spacing">;

export function MDCarouselContent({ className, spacing = "md", ...props }: MDCarouselContentProps) {
  const { orientation } = useCarousel();
  const { content } = mdCarouselStyles({ orientation, spacing });

  return (
    <BaseCarouselContent className={content({ className })} data-spacing={spacing} {...props} />
  );
}

export type MDCarouselItemProps = React.ComponentPropsWithRef<typeof BaseCarouselItem> &
  Pick<CarouselVariantProps, "size">;

export function MDCarouselItem({ className, size = "browse", ...props }: MDCarouselItemProps) {
  const { orientation } = useCarousel();
  const { item } = mdCarouselStyles({ orientation, size });

  return <BaseCarouselItem className={item({ className })} data-size={size} {...props} />;
}

export type MDCarouselControlsProps = React.ComponentPropsWithRef<"div">;

export function MDCarouselControls({ className, ...props }: MDCarouselControlsProps) {
  const { controls } = mdCarouselStyles();

  return <div className={cn(controls(), className)} data-slot="carousel-controls" {...props} />;
}
...
export function MDCarouselPrevious({
  ...
  const PreviousIcon = orientation === "vertical" ? ChevronUpIcon : ChevronLeftIcon;
...
export function MDCarouselNext({
  ...
  const { control } = mdCarouselStyles();
  const NextIcon = orientation === "vertical" ? ChevronDownIcon : ChevronRightIcon;
