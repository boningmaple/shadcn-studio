...
export default function MDCarouselDemo() {
  return (
    <MDCarousel aria-label="Travel collections" className="max-w-4xl">
      <MDCarouselContent>
        {trips.map((trip) => (
          <MDCarouselItem key={trip.title}>
            <MDCard className="h-full" interactive variant="elevated">
              <div className={`flex h-36 items-end rounded-t-[12px] p-4 ${trip.color}`}>
                <MapPinIcon className="size-8" />
              </div>
              <MDCardHeader>
                <span className="text-base leading-6 font-medium">{trip.title}</span>
