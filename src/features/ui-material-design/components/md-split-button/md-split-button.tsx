...
import {
  ...
  type MDMenuProps,
} from "@/features/ui-material-design/components/md-menu/md-menu";
import { cn } from "@/lib/utils";

export const mdSplitButtonStyles = tv({
  slots: {
    root: "inline-flex w-fit max-w-full items-center gap-1 rounded-full",
    action: "rounded-e-[12px] data-pressed:rounded-e-[8px] data-disabled:rounded-e-[12px]",
    menuButton:
...
export type MDSplitButtonVariantProps = VariantProps<typeof mdSplitButtonStyles>;
...
export function MDSplitButton({
  ...
  return (
    ...
    >
      <MDButton
        className={composeRenderProps(className, (className) => styles.action({ className }))}
        shape="square"
        size={size}
        variant={variant}
        {...props}
