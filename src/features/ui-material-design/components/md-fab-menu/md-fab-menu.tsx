...
} from "@/features/ui-material-design/components/md-fab/md-fab";
import {
  MDMenu,
  MDMenuItem,
  MDMenuTrigger,
...
export const mdFabMenuStyles = tv({
  slots: {
    ...
    triggerIcon: "transition-transform duration-200 ease-[cubic-bezier(0.2,0,0,1)]",
  },
});

type FABMenuSlotProps = Parameters<ReturnType<typeof mdFabMenuStyles>["menu"]>[0];
...
export function MDFABMenuItem({ className, ...props }: MDFABMenuItemProps) {
  ...
  return (
    <MDMenuItem
      className={composeRenderProps(className, (className) => cn(item(), className))}
      {...props}
    />
  );
}
