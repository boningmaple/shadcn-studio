...
import { Spinner } from "@/features/ui-shadcn/react-aria/spinner";

export const mdLoadingIndicatorStyles = tv({
  slots: {
    root: ["inline-flex items-center justify-center text-[#1d1b20]", "dark:text-[#e6e0e9]"],
    indicator: [
  ...
  variants: {
    ...
    appearance: {
      ...
      tonal: {
        indicator: "bg-[#e8def8] text-[#21005d] dark:bg-[#4a4458] dark:text-[#e8def8]",
      },
      surface: {
        indicator:
      ...
      inverse: {
        indicator: "bg-[#322f35] text-[#f5eff7] dark:bg-[#e6e0e9] dark:text-[#322f35]",
      },
    },
    layout: {
      inline: {
...
export type MDLoadingIndicatorVariantProps = VariantProps<typeof mdLoadingIndicatorStyles>;
type LoadingIndicatorSlotProps = Parameters<ReturnType<typeof mdLoadingIndicatorStyles>["root"]>[0];

export function mdLoadingIndicatorVariants() {
  const styles = mdLoadingIndicatorStyles();
...
export type MDLoadingIndicatorProps = React.ComponentProps<"div"> &
  VariantProps<typeof mdLoadingIndicatorStyles> & {
    label?: React.ReactNode;
  };

export function MDLoadingIndicator({
  ...
  size = "lg",
  ...props
}: MDLoadingIndicatorProps) {
  const { indicator, label: labelClassName, root, spinner } = mdLoadingIndicatorVariants();
  const visibleLabel = children ?? label;
  const resolvedAriaLabel = ariaLabel ?? (visibleLabel === undefined ? "Loading" : undefined);
