...
type NavigationRailSlotProps = Parameters<ReturnType<typeof mdNavigationRailStyles>["rail"]>[0];
...
export function MDNavigationRailHeader({ className, ...props }: MDNavigationRailHeaderProps) {
  const { header } = mdNavigationRailVariants();

  return <div className={header({ className })} data-slot="navigation-rail-header" {...props} />;
}
...
export type MDNavigationRailItemProps = Omit<RACLinkProps, "children" | "className"> & {
