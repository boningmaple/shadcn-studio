...
} from "@/features/ui-material-design/components/md-toggle-button/md-toggle-button";
...
export type MDSegmentedButtonVariantProps = VariantProps<typeof mdSegmentedButtonStyles>;
type SegmentedButtonSlotProps = Parameters<ReturnType<typeof mdSegmentedButtonStyles>["group"]>[0];
...
const SegmentedButtonStyleContext = React.createContext<SegmentedButtonStyleContextValue>({
  density: "default",
  equalWidth: false,
});

export type MDSegmentedButtonGroupProps = React.ComponentPropsWithRef<typeof RACToggleButtonGroup> &
  MDSegmentedButtonVariantProps;
...
export type MDSegmentedButtonProps = Omit<MDToggleButtonProps, "shape" | "size" | "variant"> & {
...
export function MDSegmentedButton({
  ...
  return (
    <MDToggleButton
      className={composeRenderProps(className, (className) => button({ className }))}
