...
type SnackbarSlotProps = Parameters<ReturnType<typeof mdSnackbarStyles>["root"]>[0];
...
export type MDSnackbarRegionProps<T extends MDSnackbarContent = MDSnackbarContent> = Omit<
  RACToastRegionProps<T>,
  "children" | "className"
> & {
...
export function MDSnackbarRegion<T extends MDSnackbarContent = MDSnackbarContent>({
  children,
  className,
  ...props
}: MDSnackbarRegionProps<T>) {
  ...
  return (
    <RACToastRegion
      ...
      {...props}
    >
      {children ?? (({ toast }: { toast: QueuedToast<T> }) => <MDSnackbar toast={toast} />)}
    </RACToastRegion>
  );
}

export type MDSnackbarProps<T extends MDSnackbarContent = MDSnackbarContent> = Omit<
  RACToastProps<T>,
  "children" | "className"
> & {
