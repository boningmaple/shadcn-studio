...
export const mdSearchStyles = tv({
  slots: {
    barRoot: [
    ...
    triggerLabel: "relative z-10 min-w-0 flex-1 truncate text-base leading-6 font-normal",
    viewOverlay: [
    ...
    empty: "px-6 py-8 text-center text-sm leading-5 text-[#49454f] dark:text-[#cac4d0]",
    resultItem: [
    ...
    resultSupportingText: "line-clamp-2 text-sm leading-5 text-[#49454f] dark:text-[#cac4d0]",
    resultEnd:
  ...
  variants: {
    ...
    placement: {
      docked: {
        ...
        viewModal: "mx-auto max-h-[min(36rem,calc(100vh-6rem))] w-full max-w-xl rounded-[28px]",
      },
      fullscreen: {
...
type SearchSlotProps = Parameters<ReturnType<typeof mdSearchStyles>["barRoot"]>[0];

export function mdSearchVariants() {
  const styles = mdSearchStyles();
  ...
  return {
    ...
    resultSupportingText: (props?: SearchSlotProps) => styles.resultSupportingText(props),
...
type SearchStyleProps = VariantProps<typeof mdSearchStyles>;

export type MDSearchBarProps = Omit<RACSearchFieldProps, "children" | "className"> &
...
export function MDSearchBar({
  ...
  const { barClear, barContainer, barInput, barLeading, barRoot, barTrailing } = mdSearchVariants();
  const resolvedAriaLabel = ariaLabel ?? (ariaLabelledBy === undefined ? placeholder : undefined);

  return (
    <RACSearchField
      aria-label={resolvedAriaLabel}
      aria-labelledby={ariaLabelledBy}
      className={composeRenderProps(className, (className) => barRoot({ className }))}
      data-slot="search-bar"
      data-variant={barVariant}
      {...props}
    >
      <div className={barContainer({ barVariant })} data-slot="search-bar-container">
        ...
        <RACInput className={barInput()} data-slot="search-bar-input" placeholder={placeholder} />
        {showClearButton ? (
          <MDIconButton
...
export type MDSearchViewProps<T extends object = object> = Omit<
...
export function MDSearchView<T extends object = object>({
  children,
  className,
  defaultOpen = false,
  dialogLabel = "Search",
  ...
  return (
    <RACDialogTrigger isOpen={isOpen} onOpenChange={setOpen}>
      <RACButton className={trigger({ className })} data-slot="search-view-trigger">
        <SearchIcon aria-hidden="true" className={triggerIcon()} />
        <span className={triggerLabelClassName()}>{triggerLabel ?? placeholder}</span>
      </RACButton>
      <RACModalOverlay className={viewOverlay({ placement })} isDismissable={isDismissable}>
        <RACModal
          className={viewModal({ className: modalClassName, placement })}
          data-slot="search-view-modal"
        >
          <RACDialog aria-label={dialogLabel} className={viewDialog()} data-slot="search-view">
            <RACAutocomplete {...autocompleteProps}>
              ...
              <RACListBox
                ...
                items={items}
                renderEmptyState={
                  renderEmptyState ?? (() => <div className={empty()}>No results</div>)
                }
...
type SearchResultSlot = React.ReactNode | ((props: ListBoxItemRenderProps) => React.ReactNode);

export type MDSearchViewItemProps<T extends object = object> = Omit<
...
export function MDSearchViewItem<T extends object = object>({
  children,
  className,
  end: endContent,
  headline,
  ...
  const resolvedTextValue =
    textValue ??
    (typeof headline === "string" ? headline : typeof children === "string" ? children : undefined);
  const resolvedItemLines = itemLines ?? (supportingText === undefined ? "one" : "two");
      ...
      {composeRenderProps(children, (children, renderProps) => {
        ...
        return (
          <>
            ...
            <span className={resultContent()} data-slot="search-view-item-content">
              {headlineContent === undefined || headlineContent === null ? null : (
                <span className={resultHeadline()} data-slot="search-view-item-headline">
                  {headlineContent}
                </span>
              )}
