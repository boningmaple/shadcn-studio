...
import {
  MDDivider,
  type MDDividerProps,
} from "@/features/ui-material-design/components/md-divider/md-divider";
...
type ToolbarSlotProps = Parameters<ReturnType<typeof mdToolbarStyles>["root"]>[0];
...
export function MDToolbarTitle({ className, ...props }: MDToolbarTitleProps) {
  ...
  return <h2 className={title({ className })} data-slot="toolbar-title" {...props} />;
}
...
export function MDToolbarSpacer({ className, ...props }: MDToolbarSpacerProps) {
  ...
  return <div className={spacer({ className })} data-slot="toolbar-spacer" {...props} />;
}

export type MDToolbarDividerProps = Omit<MDDividerProps, "orientation">;

export function MDToolbarDivider({ className, ...props }: MDToolbarDividerProps) {
