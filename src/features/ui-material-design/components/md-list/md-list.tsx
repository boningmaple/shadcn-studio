...
export const mdListStyles = tv({
  slots: {
    ...
    overline: "truncate text-xs leading-4 font-medium text-[#49454f] dark:text-[#cac4d0]",
    headline: "truncate text-base leading-6 font-normal text-current",
    supportingText: "line-clamp-2 text-sm leading-5 text-[#49454f] dark:text-[#cac4d0]",
...
type ListSlotProps = Parameters<ReturnType<typeof mdListStyles>["list"]>[0];

export function mdListVariants() {
  const styles = mdListStyles();
  ...
  return {
    ...
    trailingSupportingText: (props?: ListSlotProps) => styles.trailingSupportingText(props),
  };
}

export type MDListProps<T extends object = object> = Omit<RACGridListProps<T>, "className"> &
  Pick<VariantProps<typeof mdListStyles>, never> & {
    className?: RACGridListProps<T>["className"];
  };

export function MDList<T extends object = object>({ className, ...props }: MDListProps<T>) {
  const { list } = mdListVariants();
...
type ListItemSlot = React.ReactNode | ((props: GridListItemRenderProps) => React.ReactNode);

export type MDListItemProps<T extends object = object> = Omit<
  RACGridListItemProps<T>,
  "children" | "className"
> &
  Omit<VariantProps<typeof mdListStyles>, "hasEnd" | "hasStart"> & {
...
export function MDListItem<T extends object = object>({
  children,
  className,
  end: endContent,
  headline,
  ...
  const hasEnd = endContent !== undefined || trailingSupportingText !== undefined;
          ...
          <>
            ...
            <span className={content()} data-slot="list-item-content">
              {overline === undefined ? null : (
                <span className={overlineClassName()} data-slot="list-item-overline">
                  {overline}
                </span>
              )}
              {headlineContent === undefined || headlineContent === null ? null : (
                <span className={headlineClassName()} data-slot="list-item-headline">
                  {headlineContent}
                </span>
              )}
...
export type MDListSectionProps<T extends object = object> = Omit<
...
export function MDListSection<T extends object = object>({
  children,
  className,
  title,
  ...props
}: MDListSectionProps<T>) {
  const { section, sectionHeader } = mdListVariants();

  return (
    <RACGridListSection className={section({ className })} data-slot="list-section" {...props}>
      {title === undefined ? null : (
        <RACGridListHeader className={sectionHeader()} data-slot="list-section-header">
          {title}
        </RACGridListHeader>
      )}
      {children}
...
function renderListItemSlot(slot: ListItemSlot | undefined, renderProps: GridListItemRenderProps) {
