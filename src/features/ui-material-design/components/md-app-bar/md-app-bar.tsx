...
} from "@/features/ui-material-design/components/md-search/md-search";
import {
  MDToolbar,
  type MDToolbarProps,
} from "@/features/ui-material-design/components/md-toolbar/md-toolbar";
...
export function MDAppBar({
  ...
  return (
    <MDToolbar
      className={composeRenderProps(className, (className) => root({ className, elevation, size }))}
      data-size={size}
      data-slot="app-bar"
      elevation="flat"
      shape="edge"
...
export function MDAppBarTitle({ className, ...props }: MDAppBarTitleProps) {
  ...
  return <h1 className={title({ className })} data-slot="app-bar-title" {...props} />;
}
...
export function MDAppBarActions({ className, ...props }: MDAppBarActionsProps) {
  ...
  return <div className={actions({ className })} data-slot="app-bar-actions" {...props} />;
}
...
export function MDSearchAppBar({ children, searchProps, ...props }: MDSearchAppBarProps) {
  const { search } = mdAppBarVariants();
  const { className: searchClassName, ...resolvedSearchProps } = searchProps ?? {};

  return (
    <MDAppBar {...props}>
      <MDSearchBar
        className={composeRenderProps(searchClassName, (className) => search({ className }))}
        {...resolvedSearchProps}
      />
      {children}
    </MDAppBar>
