...
import { MDBadge, MDBadgeAnchor } from "@/features/ui-material-design/components/md-badge/md-badge";

export const mdNavigationBarStyles = tv({
  slots: {
    bar: [
...
type NavigationBarSlotProps = Parameters<ReturnType<typeof mdNavigationBarStyles>["bar"]>[0];

export function mdNavigationBarVariants() {
  const styles = mdNavigationBarStyles();
...
export type MDNavigationBarProps = React.ComponentPropsWithRef<"nav">;

export function MDNavigationBar({ className, ...props }: MDNavigationBarProps) {
  const { bar } = mdNavigationBarVariants();
...
export type MDNavigationBarBadge = React.ReactElement | number | string | true;

export type MDNavigationBarItemProps = Omit<RACLinkProps, "children" | "className"> & {
  "aria-current"?: React.AriaAttributes["aria-current"];
  badge?: MDNavigationBarBadge | null;
...
export function MDNavigationBarItem({
  ...
  const { icon, indicator, item, label } = mdNavigationBarVariants();
  const renderedIcon =
    typeof iconContent === "function" ? iconContent({ isSelected }) : iconContent;
  ...
  return (
    ...
    >
      <span className={indicator()} data-slot="navigation-bar-indicator">
        <MDBadgeAnchor className={icon()} data-slot="navigation-bar-icon">
          {renderedIcon}
          {renderBadge(badge)}
        </MDBadgeAnchor>
      </span>
...
function renderBadge(badge: MDNavigationBarItemProps["badge"]) {
