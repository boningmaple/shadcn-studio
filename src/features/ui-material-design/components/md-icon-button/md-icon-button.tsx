...
import {
  MDButton,
  type MDButtonProps,
} from "@/features/ui-material-design/components/md-button/md-button";
import {
  MDToggleButton,
...
export const mdIconButtonVariants = tv({
  slots: {
    root: [
...
type IconButtonStyleProps = VariantProps<typeof mdIconButtonVariants>;

export type MDIconButtonProps = Omit<MDButtonProps, "children" | "shape" | "size" | "variant"> &
  Omit<IconButtonStyleProps, "kind"> & {
    children: MDButtonProps["children"];
  };
...
export function MDIconButton({
  ...
  const styles = mdIconButtonVariants({
    kind: "default",
    variant,
    size,
    width,
  ...
  return (
    <MDButton
      className={composeRenderProps(className, (className) => cn(rootClassName, className))}
      data-shape={shape}
      data-size={size}
...
export type MDToggleIconButtonProps = Omit<
  MDToggleButtonProps,
  "children" | "shape" | "size" | "variant"
> &
  Omit<IconButtonStyleProps, "kind"> & {
    children: MDToggleButtonProps["children"];
  };

export function MDToggleIconButton({
  className,
  children,
  variant = "filled",
  size = "sm",
