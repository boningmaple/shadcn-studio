...
const levelOneShadow = "shadow-[0_1px_2px_0_rgb(0_0_0/0.3),0_1px_3px_1px_rgb(0_0_0/0.15)]";
...
export function mdChipVariants({
  ...
  return {
    ...
    selectionIndicator: (props?: ChipSlotProps) => resolve().selectionIndicator(props),
...
export function MDChipGroup({
  ...
  const effectiveSelectionMode = variant === "filter" ? (selectionMode ?? "multiple") : "none";
...
export function MDChipList<T extends object>({ className, ...props }: MDChipListProps<T>) {
...
export function MDChip({
  ...
  const { surface: surfaceVariant, variant } = React.useContext(ChipStyleContext);
      ...
      {composeRenderProps(children, (children, { isSelected }) => (
        <span className={surface()} data-slot="chip-surface">
          {variant === "filter" && isSelected ? (
            <span className={selectionIndicator()} data-slot="chip-selection-indicator">
