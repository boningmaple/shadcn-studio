...
import {
  ...
  MDChipGroup,
  MDChipList,
} from "@/features/ui-material-design/components/md-chips/md-chips";

export default function MDChipsDemo() {
  const [amenities, setAmenities] = React.useState<Selection>(new Set(["wifi", "parking"]));
  const [sortOrder, setSortOrder] = React.useState<Selection>(new Set(["popular"]));

  return (
    <div className="grid w-full gap-6">
      <div className="grid gap-2">
        <p className="text-center text-sm font-medium text-muted-foreground">Multiple selection</p>
        <MDChipGroup
      ...
      <div className="grid gap-2">
        <p className="text-center text-sm font-medium text-muted-foreground">Single selection</p>
        <MDChipGroup
      ...
      <p aria-live="polite" className="text-center text-sm text-muted-foreground">
        Amenities: {formatSelection(amenities)}. Sort: {formatSelection(sortOrder)}.
      </p>
    </div>
  );
}
