import {
  ...
  AvatarGroupCount,
  AvatarImage,
} from "@/features/ui-shadcn/react-aria/avatar";
...
export default function MDAvatarDemo() {
  return (
    <div className="flex max-w-full items-center gap-3 rounded-full border bg-background px-3 py-2 shadow-sm">
      <AvatarGroup aria-label="Community members" className="-space-x-1.5" role="group">
        {people.map((person) => (
      ...
      <p className="min-w-0 text-sm text-muted-foreground">
        Loved by <strong className="font-semibold text-foreground">10K+</strong> developers.
      </p>
    </div>
  );
}
