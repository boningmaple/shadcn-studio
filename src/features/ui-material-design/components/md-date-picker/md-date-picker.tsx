...
import { MDIconButton } from "@/features/ui-material-design/components/md-icon-button/md-icon-button";

export const mdDatePickerStyles = tv({
  slots: {
    root: [
    ...
    input: "relative z-10 flex min-w-0 flex-1 items-center py-3 text-base leading-6",
    segment: [
    ...
    rangeFieldLabel: "relative z-10 text-xs leading-4 text-[#49454f] dark:text-[#cac4d0]",
    rangeSeparator: "hidden text-sm leading-5 text-[#49454f] dark:text-[#cac4d0] sm:block",
    popover: [
    ...
    headerCell: "h-8 text-center text-xs leading-4 font-medium text-[#49454f] dark:text-[#cac4d0]",
    cell: [
    ...
    modalHeader: "grid gap-1 border-b border-[#cac4d0] px-6 pt-5 pb-4 dark:border-[#49454f]",
    modalSupporting: "text-sm leading-5 text-[#49454f] dark:text-[#cac4d0]",
    modalTitle: "font-heading text-2xl leading-8 font-normal tracking-normal text-current",
...
type DatePickerSlotProps = Parameters<ReturnType<typeof mdDatePickerStyles>["root"]>[0];

export function mdDatePickerVariants() {
  const styles = mdDatePickerStyles();

  return {
    calendar: (props?: DatePickerSlotProps) => styles.calendar(props),
    calendarGrid: (props?: DatePickerSlotProps) => styles.calendarGrid(props),
    calendarHeader: (props?: DatePickerSlotProps) => styles.calendarHeader(props),
    calendarHeading: (props?: DatePickerSlotProps) => styles.calendarHeading(props),
    ...
    fieldContainer: (props?: DatePickerSlotProps) => styles.fieldContainer(props),
    ...
    modalInputContent: (props?: DatePickerSlotProps) => styles.modalInputContent(props),
    modalOverlay: (props?: DatePickerSlotProps) => styles.modalOverlay(props),
    modalSupporting: (props?: DatePickerSlotProps) => styles.modalSupporting(props),
    ...
    rangeFieldLabel: (props?: DatePickerSlotProps) => styles.rangeFieldLabel(props),
    rangeFields: (props?: DatePickerSlotProps) => styles.rangeFields(props),
    rangeSeparator: (props?: DatePickerSlotProps) => styles.rangeSeparator(props),
    root: (props?: DatePickerSlotProps) => styles.root(props),
    segment: (props?: DatePickerSlotProps) => styles.segment(props),
  };
}

type DatePickerStyleProps = VariantProps<typeof mdDatePickerStyles>;
type VisibleMonths = "one" | "two";

export type MDDatePickerProps<T extends DateValue = DateValue> = Omit<
...
export function MDDatePicker<T extends DateValue = DateValue>({
  ...
  const {
    ...
    popoverDialog,
    root,
  } = mdDatePickerVariants();

  return (
    <RACDatePicker
      aria-label={
        ariaLabel ??
        (ariaLabelledBy === undefined && label === undefined ? "Select date" : undefined)
      }
      aria-labelledby={ariaLabelledBy}
    ...
    >
      ...
      <RACGroup className={fieldContainer()} data-slot="date-picker-field-container">
        <DatePickerInput />
        <MDIconButton aria-label="Open calendar" size="xs" variant="standard">
          <CalendarDaysIcon />
        </MDIconButton>
      </RACGroup>
...
export type MDDateRangePickerProps<T extends DateValue = DateValue> = Omit<
...
export function MDDateRangePicker<T extends DateValue = DateValue>({
  "aria-label": ariaLabel,
  "aria-labelledby": ariaLabelledBy,
  className,
  description,
  ...
  return (
    <RACDateRangePicker
      aria-label={
        ariaLabel ??
        (ariaLabelledBy === undefined && label === undefined ? "Select date range" : undefined)
      }
      aria-labelledby={ariaLabelledBy}
      className={composeRenderProps(className, (className) => root({ className }))}
    ...
    >
      {label === undefined ? null : (
        <RACLabel className={labelClassName()} data-slot="date-range-picker-label">
          {label}
        </RACLabel>
      )}
      ...
      <RACPopover
        ...
        offset={8}
      >
        <RACDialog className={popoverDialog()} data-slot="date-range-picker-dialog">
...
export type MDModalDatePickerProps<T extends DateValue = DateValue> = Omit<
  RACCalendarProps<T>,
  "children" | "className"
> &
...
export function MDModalDatePicker<T extends DateValue = DateValue>({
  className,
  ...
  return (
    <RACDialogTrigger>
      <MDButton className={className} variant="tonal">
        {triggerLabel}
      </MDButton>
      <RACModalOverlay className={modalOverlay()} isDismissable>
        <RACModal className={modal({ months: visibleMonths })} data-slot="modal-date-picker-modal">
          <RACDialog
          ...
          >
            ...
            <div className={modalContent()} data-slot="modal-date-picker-content">
              <RACCalendar
                className={undefined}
            ...
            <div className={modalActions()} data-slot="modal-date-picker-actions">
...
export type MDModalDateInputProps<T extends DateValue = DateValue> = Omit<
  RACDateFieldProps<T>,
  "children" | "className"
> & {
  className?: string;
...
export function MDModalDateInput<T extends DateValue = DateValue>({
  "aria-label": ariaLabel,
  "aria-labelledby": ariaLabelledBy,
  className,
  description,
  ...
  return (
    <RACDialogTrigger>
      <MDButton className={className} variant="outlined">
        {triggerLabel}
      </MDButton>
      <RACModalOverlay className={modalOverlay()} isDismissable>
        <RACModal className={modal()} data-slot="modal-date-input-modal">
          ...
          >
            ...
            <div className={modalInputContent()} data-slot="modal-date-input-content">
              <RACDateField
                aria-label={
                  ariaLabel ??
                  (ariaLabelledBy === undefined && label === undefined ? "Date" : undefined)
                }
                aria-labelledby={ariaLabelledBy}
                className={root()}
                {...props}
              >
                ...
                {errorMessage === undefined ? null : (
                  <RACFieldError className={error()}>{errorMessage}</RACFieldError>
                )}
              </RACDateField>
            </div>
            <div className={modalActions()} data-slot="modal-date-input-actions">
              <MDButton slot="close" variant="text">
                Cancel
              </MDButton>
              <MDButton slot="close" variant="text">
...
function DatePickerInput({ slot }: { slot?: string }) {
  const { input, segment } = mdDatePickerVariants();

  return (
    <RACDateInput className={input()} data-slot="date-picker-input" slot={slot}>
      {(segmentValue) => <RACDateSegment className={segment()} segment={segmentValue} />}
    </RACDateInput>
  );
}
...
function RangeCalendarSurface({ visibleMonths }: { visibleMonths: VisibleMonths }) {
  return (
    <RACRangeCalendar
      className={undefined}
      visibleDuration={{ months: visibleMonths === "two" ? 2 : 1 }}
...
function CalendarContent({ visibleMonths }: { visibleMonths: VisibleMonths }) {
  const { calendar, calendarGrid, calendarHeader, calendarHeading, cell, headerCell, months } =
    mdDatePickerVariants();
  ...
  return (
    <div className={calendar()} data-slot="date-picker-calendar">
      <div className={calendarHeader()} data-slot="date-picker-calendar-header">
        <MDIconButton aria-label="Previous month" size="xs" slot="previous" variant="standard">
          <ChevronLeftIcon />
        </MDIconButton>
        <RACCalendarHeading
        ...
        />
        <MDIconButton aria-label="Next month" size="xs" slot="next" variant="standard">
          <ChevronRightIcon />
        </MDIconButton>
      </div>
        ...
        {Array.from({ length: monthCount }, (_, index) => (
          ...
          >
            <RACCalendarGridHeader>
              {(day) => (
                <RACCalendarHeaderCell className={headerCell()}>{day}</RACCalendarHeaderCell>
              )}
            </RACCalendarGridHeader>
            <RACCalendarGridBody>
