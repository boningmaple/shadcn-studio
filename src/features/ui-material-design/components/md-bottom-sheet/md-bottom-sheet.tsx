...
} from "@/features/ui-material-design/components/md-icon-button/md-icon-button";
import {
  Drawer,
  DrawerClose,
  DrawerContent,
  ...
  DrawerTitle,
  DrawerTrigger,
} from "@/features/ui-shadcn/react-aria/drawer";
...
export const mdBottomSheetStyles = tv({
  ...
  variants: {
    elevation: {
      modal: {
        content: "shadow-[0_-4px_8px_3px_rgb(0_0_0/0.15),0_-1px_3px_0_rgb(0_0_0/0.3)]",
      },
      standard: {
        content: "shadow-[0_-1px_2px_0_rgb(0_0_0/0.3),0_-1px_3px_1px_rgb(0_0_0/0.15)]",
      },
    },
    height: {
      auto: {
...
type BottomSheetSlotProps = Parameters<ReturnType<typeof mdBottomSheetStyles>["content"]>[0];
...
export function MDBottomSheet({
  ...
  return <Drawer showSwipeHandle={showDragHandle} swipeDirection={swipeDirection} {...props} />;
}

export type MDBottomSheetTriggerProps = React.ComponentPropsWithRef<typeof DrawerTrigger>;
...
export function MDBottomSheetContent({
  ...
  return <DrawerContent className={content({ className, elevation, height, inset })} {...props} />;
}

export type MDBottomSheetHeaderProps = React.ComponentPropsWithRef<typeof DrawerHeader>;

export function MDBottomSheetHeader({ className, ...props }: MDBottomSheetHeaderProps) {
...
export function MDBottomSheetTitle({ className, ...props }: MDBottomSheetTitleProps) {
...
export function MDBottomSheetDescription({ className, ...props }: MDBottomSheetDescriptionProps) {
  const { description } = mdBottomSheetVariants();

  return <DrawerDescription className={description({ className })} {...props} />;
}

export type MDBottomSheetBodyProps = React.ComponentPropsWithRef<"div">;

export function MDBottomSheetBody({ className, ...props }: MDBottomSheetBodyProps) {
  const { body } = mdBottomSheetVariants();

  return <div className={body({ className })} data-slot="bottom-sheet-body" {...props} />;
}

export type MDBottomSheetFooterProps = React.ComponentPropsWithRef<typeof DrawerFooter>;

export function MDBottomSheetFooter({ className, ...props }: MDBottomSheetFooterProps) {
...
export function MDBottomSheetClose({ className, ...props }: MDBottomSheetCloseProps) {
  return <DrawerClose className={cn(className)} {...props} />;
}

export type MDBottomSheetCloseButtonProps = Omit<MDIconButtonProps, "children"> & {
    ...
    <DrawerClose
      render={
        <MDIconButton
          ...
          className={composeRenderProps(className, (className) => cn(close(), className))}
          size={size}
          variant={variant}
          {...props}
        >
