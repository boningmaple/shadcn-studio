...
export const mdTimePickerStyles = tv({
  slots: {
    root: [
    ...
    modalSupporting: "flex items-center gap-2 text-sm leading-5 text-[#49454f] dark:text-[#cac4d0]",
...
type TimePickerSlotProps = Parameters<ReturnType<typeof mdTimePickerStyles>["root"]>[0];

export function mdTimePickerVariants() {
  const styles = mdTimePickerStyles();
  ...
  return {
    ...
    modalSupporting: (props?: TimePickerSlotProps) => styles.modalSupporting(props),
...
export type MDTimePickerInputProps<T extends TimeValue = TimeValue> = Omit<
  RACTimeFieldProps<T>,
  "children" | "className"
> & {
...
export function MDTimePickerInput<T extends TimeValue = TimeValue>({
  ...
  const {
    ...
    root,
    segment,
  } = mdTimePickerVariants();

  return (
    <RACTimeField
      aria-label={
        ariaLabel ??
        (ariaLabelledBy === undefined && label === undefined ? "Select time" : undefined)
      }
      aria-labelledby={ariaLabelledBy}
    ...
    >
      ...
      <RACTimeInput className={input()} data-slot="time-picker-field">
        {(segmentValue) => <RACTimeSegment className={segment()} segment={segmentValue} />}
      </RACTimeInput>
...
export type MDModalTimePickerInputProps<T extends TimeValue = TimeValue> =
  MDTimePickerInputProps<T> & {
...
export function MDModalTimePickerInput<T extends TimeValue = TimeValue>({
  supportingText = "Input time picker",
  title = "Enter time",
  triggerLabel = "Open time input",
  ...props
}: MDModalTimePickerInputProps<T>) {
  const {
    modal,
    modalActions,
    modalContent,
  ...
  return (
    <RACDialogTrigger>
      <MDButton variant="tonal">{triggerLabel}</MDButton>
      <RACModalOverlay className={modalOverlay()} isDismissable>
        <RACModal className={modal()} data-slot="modal-time-picker-modal">
          ...
          >
            ...
            <div className={modalContent()} data-slot="modal-time-picker-content">
              <MDTimePickerInput {...props} />
            </div>
            <div className={modalActions()} data-slot="modal-time-picker-actions">
