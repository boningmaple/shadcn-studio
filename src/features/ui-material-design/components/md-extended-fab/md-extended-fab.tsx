...
export const mdExtendedFABButtonVariants = tv({
  base: [
  ...
  variants: {
    color: {
      surface: ["bg-[#f7f2fa] text-[#6750a4]", "dark:bg-[#211f26] dark:text-[#d0bcff]"],
      primary: ["bg-[#eaddff] text-[#21005d]", "dark:bg-[#4f378b] dark:text-[#eaddff]"],
      secondary: ["bg-[#e8def8] text-[#1d192b]", "dark:bg-[#4a4458] dark:text-[#e8def8]"],
      tertiary: ["bg-[#ffd8e4] text-[#31111d]", "dark:bg-[#633b48] dark:text-[#ffd8e4]"],
    },
    lowered: {
...
export type MDExtendedFABButtonProps = Omit<MDButtonProps, "shape" | "size" | "variant"> &
  VariantProps<typeof mdExtendedFABButtonVariants>;

export function MDExtendedFABButton({
  className,
  color = "surface",
  lowered = false,
  ...props
}: MDExtendedFABButtonProps) {
  return (
    <MDButton
      className={composeRenderProps(className, (className) =>
        mdExtendedFABButtonVariants({ className, color, lowered }),
      )}
      data-color={color}
