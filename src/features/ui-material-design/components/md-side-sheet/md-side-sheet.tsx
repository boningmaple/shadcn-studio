...
import {
  ...
  SheetTitle,
  SheetTrigger,
} from "@/features/ui-shadcn/react-aria/sheet";
import { Sidebar, SidebarProvider } from "@/features/ui-shadcn/react-aria/sidebar";
...
type SideSheetSlotProps = Parameters<ReturnType<typeof mdSideSheetStyles>["standard"]>[0];
...
export type MDSideSheetTriggerProps = React.ComponentPropsWithRef<typeof SheetTrigger>;
...
export function MDStandardSideSheet({
  ...
  return (
    <SidebarProvider
      ...
      {...props}
    >
      <Sidebar className={standard({ className })} collapsible="none" data-side={side} side={side}>
        {children}
      </Sidebar>
    </SidebarProvider>
  );
...
export function MDSideSheetHeader({ className, ...props }: MDSideSheetHeaderProps) {
  const { header } = mdSideSheetVariants();

  return <div className={header({ className })} data-slot="side-sheet-header" {...props} />;
}
...
export function MDSideSheetTitle({ className, ...props }: MDSideSheetTitleProps) {
...
export function MDSideSheetDescription({ className, ...props }: MDSideSheetDescriptionProps) {
...
export function MDSideSheetBody({ className, ...props }: MDSideSheetBodyProps) {
  ...
  return <div className={body({ className })} data-slot="side-sheet-body" {...props} />;
}

export type MDSideSheetFooterProps = React.ComponentPropsWithRef<"div">;

export function MDSideSheetFooter({ className, ...props }: MDSideSheetFooterProps) {
  const { footer } = mdSideSheetVariants();

  return <div className={footer({ className })} data-slot="side-sheet-footer" {...props} />;
}

export type MDSideSheetDividerProps = React.ComponentPropsWithRef<"div">;

export function MDSideSheetDivider({ className, ...props }: MDSideSheetDividerProps) {
  const { divider } = mdSideSheetVariants();

  return <div className={divider({ className })} data-slot="side-sheet-divider" {...props} />;
}

export type MDSideSheetCloseButtonProps = Omit<MDIconButtonProps, "children"> & {
