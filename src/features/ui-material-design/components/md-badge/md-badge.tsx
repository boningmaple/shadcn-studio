...
import { mdGetBadgeSize } from "@/features/ui-material-design/components/md-badge/md-badge-state";
...
export function MDBadge({ children, className, ref, ...props }: MDBadgeProps) {
  ...
  return (
    <span
      ...
      {...props}
    >
      {size === "large" ? <span data-slot="badge-label">{children}</span> : null}
    </span>
  );
}
...
export function MDBadgeAnchor({ className, ref, ...props }: MDBadgeAnchorProps) {
  return (
    <span
      ref={ref}
      className={cn(
