...
// Define the required shape of a component catalog entry.
// Every ComponentEntry object must contain all fields listed below.
export type ComponentEntry = {
  /**
   * Prefix used when naming generated files or code artifacts.
   * For example: "date-picker" might produce "date-picker-demo.tsx".
   */
  codeArtifactPrefix: string;

  /**
   * Singular noun used when referring to one demo in written text.
   * For example: "example", "variant", or "pattern".
   */
  demoNoun: string;

  // The demos belonging to this component.
  // `readonly` prevents code from adding, removing, or replacing array
  // elements through a ComponentEntry value.
  demos: readonly Demo[];

  // Human-readable summary of what the component does.
  description: string;

  // URL or application route for the component's documentation page.
  href: string;

  // Human-readable component name, such as "Date Picker".
  name: string;

  // Machine-friendly identifier for the section containing this component.
  sectionId: string;

  // Human-readable section heading, such as "Form Components".
  sectionTitle: string;

  // URL-safe identifier for the component, such as "date-picker".
  slug: string;
};
...
export type DemoIdOf<TSlug extends ComponentSlug> = ComponentEntryFor<TSlug>["demos"][number]["id"];

const bySlug = new Map<string, ComponentEntry>(
  components.map((component) => [component.slug, component]),
);

export function getComponent<TSlug extends ComponentSlug>(slug: TSlug): ComponentEntryFor<TSlug> {
