...
import type { SearchHit } from "../api/search.contract.ts";
import { demoAnchorId, type ComponentEntry } from "../data/registry.ts";
...
function searchRecordsOf(components: readonly ComponentEntry[]): SearchRecord[] {
 ...
 *
 * An empty query has no Hits. Nothing has been asked, and the palette answers
 * a visitor who has typed nothing with quick links of its own (ADR-0006)
