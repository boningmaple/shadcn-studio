...
export const m3SnackbarStyles = tv({
  slots: {
    region:
...
type SnackbarSlotProps = Parameters<
  ReturnType<typeof m3SnackbarStyles>["root"]
>[0];

export function m3SnackbarVariants() {
  const styles = m3SnackbarStyles();
...
export type M3SnackbarRegionProps<
  T extends M3SnackbarContent = M3SnackbarContent,
...
export function M3SnackbarRegion<
  T extends M3SnackbarContent = M3SnackbarContent,
>({ children, className, ...props }: M3SnackbarRegionProps<T>) {
  const { region } = m3SnackbarVariants();
    ...
    >
      {children ??
        (({ toast }: { toast: QueuedToast<T> }) => (
          <M3Snackbar toast={toast} />
        ))}
    </RACToastRegion>
  );
}

export type M3SnackbarProps<T extends M3SnackbarContent = M3SnackbarContent> =
  Omit<RACToastProps<T>, "children" | "className"> & {
    className?: RACToastProps<T>["className"];
  };

export function M3Snackbar<T extends M3SnackbarContent = M3SnackbarContent>({
  className,
  toast,
  ...props
}: M3SnackbarProps<T>) {
  const { action, close, content, message, root } = m3SnackbarVariants();
  ...
  return (
    ...
    >
      ...
      {snackbar.actionLabel === undefined ? null : (
        <M3Button className={action()} onPress={handleAction} variant="text">
