...
export const m3FabButtonVariants = tv({
  base: [
...
export type M3FABButtonProps = Omit<
  M3ButtonProps,
  "shape" | "size" | "variant"
> &
  VariantProps<typeof m3FabButtonVariants>;

export function M3FABButton({
  className,
  color = "surface",
  lowered = false,
  size = "medium",
  ...
  return (
    <M3Button
      className={composeRenderProps(className, (className) =>
        m3FabButtonVariants({ className, color, lowered, size }),
      )}
      data-color={color}
