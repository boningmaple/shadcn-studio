...
export type M3ChipVariantProps = VariantProps<typeof chipStyles>;
type ChipVariant = NonNullable<M3ChipVariantProps["variant"]>;
type ChipSurface = NonNullable<M3ChipVariantProps["surface"]>;
type ChipSlotProps = Parameters<ReturnType<typeof chipStyles>["group"]>[0];

export function m3ChipVariants({
  surface = "outlined",
  variant = "assist",
}: M3ChipVariantProps = {}) {
...
export type M3ChipGroupProps = Omit<
...
export function M3ChipGroup({
  ...
  const { group } = m3ChipVariants({ surface, variant });
...
export type M3ChipListProps<T extends object> = RACTagListProps<T> &
  React.RefAttributes<HTMLDivElement>;

export function M3ChipList<T extends object>({
  className,
  ...props
}: M3ChipListProps<T>) {
  const { surface, variant } = React.useContext(ChipStyleContext);
  const { list } = m3ChipVariants({ surface, variant });
...
export type M3ChipProps = React.ComponentPropsWithRef<typeof RACTag> & {
...
export function M3Chip({
  ...
  const { icon, item, label, selectionIndicator, surface } = m3ChipVariants({
