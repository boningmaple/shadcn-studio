...
export const m3TimePickerStyles = tv({
  slots: {
    root: [
...
type TimePickerSlotProps = Parameters<
  ReturnType<typeof m3TimePickerStyles>["root"]
>[0];

export function m3TimePickerVariants() {
  const styles = m3TimePickerStyles();
...
export type M3TimePickerInputProps<T extends TimeValue = TimeValue> = Omit<
  RACTimeFieldProps<T>,
  "children" | "className"
> & {
...
export function M3TimePickerInput<T extends TimeValue = TimeValue>({
...
export type M3ModalTimePickerInputProps<T extends TimeValue = TimeValue> =
  M3TimePickerInputProps<T> & {
...
export function M3ModalTimePickerInput<T extends TimeValue = TimeValue>({
  supportingText = "Input time picker",
  title = "Enter time",
  triggerLabel = "Open time input",
  ...props
}: M3ModalTimePickerInputProps<T>) {
  const {
    modal,
    modalActions,
    modalContent,
  ...
  return (
    <RACDialogTrigger>
      <M3Button variant="tonal">{triggerLabel}</M3Button>
