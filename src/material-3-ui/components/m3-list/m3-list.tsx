...
type ListSlotProps = Parameters<ReturnType<typeof m3ListStyles>["list"]>[0];

export function m3ListVariants() {
  const styles = m3ListStyles();
...
export type M3ListProps<T extends object = object> = Omit<
  RACGridListProps<T>,
  "className"
> &
  Pick<VariantProps<typeof m3ListStyles>, never> & {
    className?: RACGridListProps<T>["className"];
  };

export function M3List<T extends object = object>({
  className,
  ...props
}: M3ListProps<T>) {
  const { list } = m3ListVariants();
...
export type M3ListItemProps<T extends object = object> = Omit<
  RACGridListItemProps<T>,
  "children" | "className"
> &
  Omit<VariantProps<typeof m3ListStyles>, "hasEnd" | "hasStart"> & {
...
export function M3ListItem<T extends object = object>({
  children,
  className,
  end: endContent,
  headline,
...
export type M3ListSectionProps<T extends object = object> = Omit<
...
export function M3ListSection<T extends object = object>({
  children,
  className,
  title,
  ...props
}: M3ListSectionProps<T>) {
  const { section, sectionHeader } = m3ListVariants();
