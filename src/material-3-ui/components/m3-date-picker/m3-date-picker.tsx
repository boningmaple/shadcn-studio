...
export const m3DatePickerStyles = tv({
  slots: {
    root: [
...
type DatePickerSlotProps = Parameters<
  ReturnType<typeof m3DatePickerStyles>["root"]
>[0];

export function m3DatePickerVariants() {
  const styles = m3DatePickerStyles();
...
type DatePickerStyleProps = VariantProps<typeof m3DatePickerStyles>;
type VisibleMonths = "one" | "two";

export type M3DatePickerProps<T extends DateValue = DateValue> = Omit<
...
export function M3DatePicker<T extends DateValue = DateValue>({
  ...
  return (
    ...
    >
      ...
      >
        <DatePickerInput />
        <M3IconButton aria-label="Open calendar" size="xs" variant="standard">
          <CalendarDaysIcon />
        </M3IconButton>
      </RACGroup>
...
export type M3DateRangePickerProps<T extends DateValue = DateValue> = Omit<
...
export function M3DateRangePicker<T extends DateValue = DateValue>({
  "aria-label": ariaLabel,
  "aria-labelledby": ariaLabelledBy,
  className,
  description,
...
export type M3ModalDatePickerProps<T extends DateValue = DateValue> = Omit<
  RACCalendarProps<T>,
  "children" | "className"
> &
...
export function M3ModalDatePicker<T extends DateValue = DateValue>({
  className,
  ...
  return (
    <RACDialogTrigger>
      <M3Button className={className} variant="tonal">
        {triggerLabel}
      </M3Button>
...
export type M3ModalDateInputProps<T extends DateValue = DateValue> = Omit<
  RACDateFieldProps<T>,
  "children" | "className"
> & {
  className?: string;
...
export function M3ModalDateInput<T extends DateValue = DateValue>({
  "aria-label": ariaLabel,
  "aria-labelledby": ariaLabelledBy,
  className,
  description,
  ...
  return (
    <RACDialogTrigger>
      <M3Button className={className} variant="outlined">
        {triggerLabel}
      </M3Button>
      <RACModalOverlay className={modalOverlay()} isDismissable>
...
function DatePickerInput({ slot }: { slot?: string }) {
  const { input, segment } = m3DatePickerVariants();
