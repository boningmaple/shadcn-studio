...
export const m3SwitchStyles = tv({
  slots: {
    root: [
...
export type M3SwitchVariantProps = VariantProps<typeof m3SwitchStyles>;
type SwitchSlotProps = Parameters<ReturnType<typeof m3SwitchStyles>["root"]>[0];

export function m3SwitchVariants() {
  const styles = m3SwitchStyles();
...
export type M3SwitchProps = React.ComponentPropsWithRef<typeof RACSwitch>;

export function M3Switch({ children, className, ...props }: M3SwitchProps) {
  const { control, handle, icon, label, root, track } = m3SwitchVariants();
