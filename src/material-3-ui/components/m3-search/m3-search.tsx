...
export const m3SearchStyles = tv({
  slots: {
    barRoot: [
...
type SearchSlotProps = Parameters<
  ReturnType<typeof m3SearchStyles>["barRoot"]
>[0];

export function m3SearchVariants() {
  const styles = m3SearchStyles();
...
type SearchStyleProps = VariantProps<typeof m3SearchStyles>;

export type M3SearchBarProps = Omit<
  RACSearchFieldProps,
  "children" | "className"
> &
...
export type M3SearchViewProps<T extends object = object> = Omit<
...
export function M3SearchView<T extends object = object>({
  children,
  className,
  defaultOpen = false,
  dialogLabel = "Search",
...
export type M3SearchViewItemProps<T extends object = object> = Omit<
...
export function M3SearchViewItem<T extends object = object>({
  children,
  className,
  end: endContent,
  headline,
