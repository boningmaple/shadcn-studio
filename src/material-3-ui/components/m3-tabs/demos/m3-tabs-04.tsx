    ...
    <div className="grid w-full gap-8 md:grid-cols-2">
      {states.map((state) => (
        <div className="grid gap-2" key={state.label}>
          ...
          <M3Tabs defaultSelectedKey="active">
            <M3TabList
              aria-label={`Primary inactive tab ${state.label.toLowerCase()} state`}
            >
              <M3Tab id="active">Active</M3Tab>
              <M3Tab className={state.className} id="inactive">
                Inactive
              </M3Tab>
            </M3TabList>
          </M3Tabs>
