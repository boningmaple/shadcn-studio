...
export const m3TooltipStyles = tv({
  slots: {
    plain: [
...
type TooltipSlotProps = Parameters<
  ReturnType<typeof m3TooltipStyles>["plain"]
>[0];

export function m3TooltipVariants() {
  const styles = m3TooltipStyles();
...
export type M3TooltipTriggerProps = React.ComponentPropsWithRef<
  typeof RACTooltipTrigger
>;

export function M3TooltipTrigger({
  delay = 500,
  closeDelay = 100,
  children,
  ...props
}: M3TooltipTriggerProps) {
...
export type M3TooltipProps = Omit<
...
export type M3RichTooltipTriggerProps = RACDialogTriggerProps;

export function M3RichTooltipTrigger(props: M3RichTooltipTriggerProps) {
  return <RACDialogTrigger data-slot="rich-tooltip-trigger" {...props} />;
}

export type M3RichTooltipProps = Omit<
