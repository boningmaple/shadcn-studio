...
type NavigationRailSlotProps = Parameters<
  ReturnType<typeof m3NavigationRailStyles>["rail"]
>[0];

export function m3NavigationRailVariants() {
  const styles = m3NavigationRailStyles();
...
export type M3NavigationRailProps = Omit<
...
export function M3NavigationRail({
  ...
  const { rail } = m3NavigationRailVariants();
...
export type M3NavigationRailHeaderProps = React.ComponentPropsWithRef<"div">;

export function M3NavigationRailHeader({
  className,
  ...props
}: M3NavigationRailHeaderProps) {
  const { header } = m3NavigationRailVariants();
...
export type M3NavigationRailContentProps = React.ComponentPropsWithRef<"div"> &
  Pick<VariantProps<typeof m3NavigationRailStyles>, "alignment">;

export function M3NavigationRailContent({
  alignment = "top",
  className,
  ...props
}: M3NavigationRailContentProps) {
  const { content } = m3NavigationRailVariants();
...
export type M3NavigationRailBadge = React.ReactElement | number | string | true;

export type M3NavigationRailItemProps = Omit<
  RACLinkProps,
  "children" | "className"
> & {
  "aria-current"?: React.AriaAttributes["aria-current"];
  badge?: M3NavigationRailBadge | null;
...
export function M3NavigationRailItem({
  ...
  const { icon, indicator, item, label } = m3NavigationRailVariants();
  const renderedIcon =
  ...
  return (
    ...
    >
      <span className={indicator()} data-slot="navigation-rail-indicator">
        <M3BadgeAnchor className={icon()} data-slot="navigation-rail-icon">
          {renderedIcon}
          {renderBadge(badge)}
        </M3BadgeAnchor>
      </span>
...
function renderBadge(badge: M3NavigationRailItemProps["badge"]) {
